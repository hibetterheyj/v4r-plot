...
saveFileName = 'TB_BiCF_prec_log_star';
...
for ii = 1:length(succ)
    %     plot(x(ii),y(ii),'color',plotDrawStyle{ii}.color, 'Marker', plotDrawStyle{ii}.Marker, 'MarkerSize',12, 'LineWidth',2);
    %     scatter(x(ii),y(ii), plotDrawStyle{ii}.color, 'Marker', plotDrawStyle{ii}.Marker, 'MarkerSize',12, 'LineWidth',2);
    if ii==1
                plot(log10(fps(ii)),prec(ii),plotDrawStyle{ii}.Marker,'MarkerSize',10,'LineWidth', 4, 'Color', plotDrawStyle{ii}.color);
%         semilogx(log10(fps(ii)),succ(ii),plotDrawStyle{ii}.Marker,'MarkerSize',10,'LineWidth', 3);
    else
                plot(log10(fps(ii)),prec(ii),plotDrawStyle{ii}.Marker,'MarkerSize',10,'LineWidth', 3);
%         semilogx(log10(fps(ii)),succ(ii),plotDrawStyle{ii}.Marker,'MarkerSize',8,'LineWidth', 2);
    ...
    if ii==1
        ...
        tmpName{ii} = ['\textbf{TB-BiCF} (Ours  ', num2str(FPSSet{ii},'%.1f'), ' fps)' ];
        %     tmpName{ii} = sprintf('%.*s\n', 20 ,[TrkSet{ii}, ' (' FPSSet{ii} ')' ]);
    elseif ii==2
        tmpName{ii} = ['BiCF (2020ICRA  ', num2str(FPSSet{ii},'%.1f'), ' fps)' ];
    else
...
set(gca,'XTick',[log10(0.5),0,1,log10(30),2,3]);
set(gca,'xticklabel',{'0.5','1','10','30','100','1000'});
% 矫正 ytick 全为保留三位小数
...
needy = [0,1];
plot(needx,needy,'--','Color',[217, 83, 25]/255,'LineWidth',1);
% 竖线分割线浅绿色的线
