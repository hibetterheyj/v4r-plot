...
figure33_1 = figure(302);
set(figure33_1,'position',[0 0 700 360]);
% 21 trackers
h1=plot(order(1:24), succ(1:24), 'd',...
    'MarkerSize',8,...
    'MarkerEdgeColor',[0 0 0],... % [0,0.45,0.74]
    'MarkerFaceColor',[1 1 1]);
hold on
for ii = 1:24
    plot([order(ii) order(ii)], [0 succ(ii)],'Color',[0,0,0]);
    hold on
end
% blue
h1=plot(order(21), succ(21), 'd',...
    'MarkerSize',8,...
    'MarkerEdgeColor',[0 0 1],... % [0,0.45,0.74]
    'MarkerFaceColor',[0 0 1]);
hold on
for ii = 21:21
    plot([order(ii) order(ii)], [0 succ(ii)],'Color',[0,0,1]);
    hold on
end
% green
h1=plot(order(22), succ(22), 'd',...
    'MarkerSize',8,...
    'MarkerEdgeColor',[0 1 0],... % [0,0.45,0.74]
    'MarkerFaceColor',[0 1 0]);
hold on
for ii = 22:22
    plot([order(ii) order(ii)], [0 succ(ii)],'Color',[0,1,0]);
    hold on
end
% red
h1=plot(order(24), succ(24), 'd',...
    'MarkerSize',8,...
    'MarkerEdgeColor',[1 0 0],... % [0,0.45,0.74]
    'MarkerFaceColor',[1 0 0]);
hold on
for ii = 24:24
    plot([order(ii) order(ii)], [0 succ(ii)],'Color',[1,0,0]);
    hold on
end
